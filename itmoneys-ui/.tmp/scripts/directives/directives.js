...
app.directive('reloadWhenClick', ['$location', '$route', function ($location, $route) {
    return {
        restrict: 'A',
        link: function link($scope, element, attrs) {
            var href = attrs.ngHref || element.attr('href');
...
app.directive('chart', function () {
    return {
        ...
        controller: ['$scope', '$element', '$attrs', 'TimeSeriesChart', function controller($scope, $element, $attrs, TimeSeriesChart) {
...
app.directive('sparkLine', function () {
    return {
        ...
        controller: ['$scope', '$element', '$attrs', 'SparkLineChart', function controller($scope, $element, $attrs, SparkLineChart) {
            ...
            scope.load = function (to) {
                ...
                scope.list.$promise.then(function (list) {
                    ...
                    if (scope.page.current > 1 && scope.page.current < scope.page.total) {
                        scope.pages = [scope.page.current - 1, scope.page.current, scope.page.current + 1];
                    } else if (scope.page.current == 1 && scope.page.total > 1) {
                        scope.pages = [scope.page.current, scope.page.current + 1];
                    } else if (scope.page.current == scope.page.total && scope.page.total > 1) {
                        scope.pages = [scope.page.current - 1, scope.page.current];
                    }
                });
            };
            scope.load();
