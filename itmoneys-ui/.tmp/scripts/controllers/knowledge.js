...
app.controller('KnowledgeController', ['$scope', '$rootScope', '$location', 'Knowledge', 'Util', 'DataSource', 'Metrics', 'Collector', 'Parser', function ($scope, $rootScope, $location, Knowledge, Util, DataSource, Metrics, Collector, Parser) {
    ...
    $scope.save = function (formValid) {
        ...
        var dataForm = angular.copy($scope.knowledge);
    ...
    $scope.uploadFile = function () {
        ...
        reader.onload = function (file) {
            return function (e) {
                ...
                //删除不存在的采集器
                var collectorList = $scope.collectors.filter(function (collector) {
                ...
                //删除不存在的数据源
                var datasourceList = $scope.datasources.filter(function (datasource) {
                ...
                //删除不存在的解析规则
                var parserList = $scope.resolvers.filter(function (resolvers) {
    ...
    $scope.addHostPort = function () {
        var listens = $scope.knowledge.data.hostPorts;
        if (!listens) {
    ...
    $scope.addHostEncoding = function () {
        var listens = $scope.knowledge.data.listens;
        if (!listens) {
    ...
    $scope.changeDBType = function () {
        var type = $scope.knowledge.data.protocol;
        var port = null;
        switch (type) {
            case 'mysql':
                port = 3306;
