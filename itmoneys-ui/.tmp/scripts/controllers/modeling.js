...
app.controller('ModelingController', ['$scope', '$rootScope', '$location', 'Modeling', 'Analyzer', 'Util', 'DataSource', 'Metrics', 'Collector', 'Parser', 'Writer', 'Channel', function ($scope, $rootScope, $location, Modeling, Analyzer, Util, DataSource, Metrics, Collector, Parser, Writer, Channel) {
    ...
    $scope.analyzeSel = function () {
        $scope.analyzers.find(function (item) {
    ...
    $scope.usableCheck = function () {
        var data = angular.copy($scope.modeling);
    ...
    $scope.save = function (formValid) {
        ...
        var dataForm = angular.copy($scope.modeling);
    ...
    $scope.uploadFile = function () {
        ...
        reader.onload = function (file) {
            return function (e) {
                ...
                //删除不存在的解析规则
                var analyzerList = $scope.analyzers.filter(function (analyzer) {
    ...
    $scope.addHostPort = function () {
        var listens = $scope.modeling.data.hostPorts;
        if (!listens) {
    ...
    $scope.addHostEncoding = function () {
        var listens = $scope.modeling.data.listens;
        if (!listens) {
    ...
    function setWriterForViewer(sinks) {
        if ($scope.sinks.$resolved) {
            var tempWriters = new Array($scope.sinks.length);
            if (sinks) {

                for (var i = 0, j = sinks.length; i < j; i++) {
                    var writer = sinks[i];
                    if (writer) {
        ...
        function getIndex(id) {
            ...
            for (var _i = 0, _j = $scope.sinks.length; _i < _j; _i++) {
                if ($scope.sinks[_i].id == id) {
                    data = [_i, $scope.sinks[_i]];
                    break;
                }
            }
            return data;
    ...
    function propertiesWithType() {
        ...
        for (var item in $scope.modeling.properties) {
            var v = $scope.modeling.properties[item];
            if (v && reg.test(v)) {
