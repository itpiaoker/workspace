...
import com.zkys.spark.dao.*;
import com.zkys.spark.dao.factory.DAOFactory;
import com.zkys.spark.domain.*;
import com.zkys.spark.test.MockData;
import com.zkys.spark.util.*;
...
//		sc.checkpointFile("hdfs://");
		...
		//string表示的sessionid，row表示的一条操作数据
//		sessionid123   pageid1  actiontime1....
//		sessionid123   pageid2  actiontime2....
//		sessionid456   pageid1  actiontime1....
// 		sessionid456   pageid2  actiontime2....
        JavaPairRDD<String, Row> sessionid2actionRDD = getSessionid2ActionRDD(actionRDD);
		Tuple2<String, Row> stringRowTuple2 = sessionid2actionRDD.take(1).get(0);
		System.out.println("sessionid2actionRDD:"+stringRowTuple2._1+"\t"+stringRowTuple2._2);
		/**
...
//		sessionid2actionRDD.checkpoint();
		...
		//这里是sessionid和聚合之后的session所有操作和用户信息
//		sessionid123   pageid1|pageid2  actiontime1|actiontime2....userinfo
//		sessionid456   pageid1|pageid2  actiontime1|actiontime2....userinfo
		JavaPairRDD<String, String> sessionid2AggrInfoRDD =
				aggregateBySession(sc, sqlContext, sessionid2actionRDD);
		Tuple2<String, String> stringStringTuple2 = sessionid2AggrInfoRDD.take(1).get(0);
		System.out.println("sessionid2AggrInfoRDD:"+stringStringTuple2._1+"\t"+stringStringTuple2._2);
...
//		sessionid123   pageid1  actiontime1....
//		sessionid123   pageid2  actiontime2....
		//这里的方法中用了join，实际是过滤出符合条件的session的详细信息
		JavaPairRDD<String, Row> sessionid2detailRDD = getSessionid2detailRDD(
				filteredSessionid2AggrInfoRDD, sessionid2actionRDD);
		Tuple2<String, Row> stringRowTuple21 = sessionid2detailRDD.take(1).get(0);
		System.out.println("sessionid2detailRDD:"+stringRowTuple21._1+'\t'+stringRowTuple21._2);
		 ...
		 * 不对！！！
		 *
		 * 必须把能够触发job执行的操作，放在最终写入MySQL方法之前，也就是放在
         * sessionAggrStatAccumulator.value()之前，否则就是个空
		 *
         ...
         * 	    join group都会产生数据倾斜，后面会讲一整套的解决方案！
		 * 4、无论做什么功能，性能第一
				...
				new PairFunction<Tuple2<String,Iterable<Row>>, Long, String>() {
					...
					public Tuple2<Long, String> call(Tuple2<String, Iterable<Row>> tuple)
						...
						while(iterator.hasNext()) {
                            ...
                            if(row.get(6)!=null) { clickCategoryId = row.getLong(6);}
...
//		<sessionid,(sessionid,searchKeywords,clickCategoryIds,age,professional,city,sex)>
	 ...
	 * @return
	 * sessionid2aggrInfoRDD是经过条件过滤的RDD，sessionid2actionRDD所有session的操作详细信息
	 * join之后得到的是通过筛选条件的session详细信息
	 */
...
//		sessionid2AggrInfoRDD格式为sessionid    该session所有操作的拼串
//		time2sessionidRDD  日期小时   某个session所有操作聚合
...
//				hourExtractNumber表示一个小时要抽取几个session
...
//		Map<String, Map<String, List<Integer>>> dateHourExtractMap这个已经是结果，接下来的代码是优化
		/**
		...
		/**
		 * 第四步：获取抽取出来的session的明细数据，就是得到session的原始用户行为基础数据写到session_detail表
				*/
				...
				new VoidFunction<Iterator<Tuple2<String,Tuple2<String,Row>>>>() {
					...
					public void call(
						...
						while(iterator.hasNext()) {
							...
							sessionDetail.setActionTime(row.getString(4));
							sessionDetail.setSearchKeyword(row.getString(5));
                            if(row.get(6)!=null) { sessionDetail.setClickCategoryId(row.getLong(6));}
                            if(row.get(7)!=null) { sessionDetail.setClickProductId(row.getLong(7));}
							sessionDetail.setOrderCategoryIds(row.getString(8));
							sessionDetail.setOrderProductIds(row.getString(9));  
							sessionDetail.setPayCategoryIds(row.getString(10)); 
							sessionDetail.setPayProductIds(row.getString(11));  
...
//		tmpMapRDD <品类id，品类id和点击个数的字符串拼串>
//		orderCategoryId2CountRDD <品类id，订单个数>
...
//		<品类id，品类id和点击个数的字符串拼串外拼接品类订单个数再外拼接支付的个数的字符串>
//		<1，<categoryid=1|clickcategoryid=5|ordercategoryid=7|paycategoryid=9>>
...
//	top10CategoryList	<排序的对象，品类id和各种事件个数的拼串>
...
//						Long是品类id  string是sessionid+次数，list表示的是在这个session中这个用户对不同品类
//						id点击的次数的集合
...
//		<品类id,sessionid+次数>   这里的品类id 剩下排名前10的品类了，其余的过滤掉了
