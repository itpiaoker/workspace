...
public class MongoContextFactory implements LoggerContextFactory, ShutdownCallbackRegistry {
	 ...
	 * {@link Constants#LOG4J_CONTEXT_SELECTOR}.
	 */
	 ...
	 *            the selector to use
	 */
	public MongoContextFactory(final ContextSelector selector) {
		this(selector, createShutdownCallbackRegistry());
	}

	/**
	 * Constructs a Log4jContextFactory using the ContextSelector from
	 * {@link Constants#LOG4J_CONTEXT_SELECTOR} and the provided
	 * ShutdownRegistrationStrategy.
	 *
	 * @param shutdownCallbackRegistry
	 *            the ShutdownRegistrationStrategy to use
	 * @since 2.1
	 */
	public MongoContextFactory(final ShutdownCallbackRegistry shutdownCallbackRegistry) {
		this(createContextSelector(), shutdownCallbackRegistry);
	}

	/**
	 * Constructs a Log4jContextFactory using the provided ContextSelector and
	 * ShutdownRegistrationStrategy.
	 *
	 * @param selector
	 *            the selector to use
	...
	public MongoContextFactory(final ContextSelector selector,
		...
		this.shutdownCallbackRegistry = Objects.requireNonNull(shutdownCallbackRegistry,
	...
	private static ShutdownCallbackRegistry createShutdownCallbackRegistry() {
		// TODO: this is such a common idiom it really deserves a utility method
		// somewhere
		final String registry = PropertiesUtil.getProperties()
				.getStringProperty(ShutdownCallbackRegistry.SHUTDOWN_CALLBACK_REGISTRY);
		if (registry != null) {
			try {
				return Loader.newCheckedInstanceOf(registry, ShutdownCallbackRegistry.class);
			} catch (final Exception e) {
				LOGGER.error(SHUTDOWN_HOOK_MARKER, "There was an error loading the ShutdownCallbackRegistry [{}]. "
	 ...
	 *            If true returns the current Context, if false returns the
	 *            Context appropriate for the caller if a more appropriate
